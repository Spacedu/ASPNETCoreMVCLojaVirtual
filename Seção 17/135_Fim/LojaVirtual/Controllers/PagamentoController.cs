...
namespace LojaVirtual.Controllers
{
    ...
    public class PagamentoController : BaseController
    {
        ...
        {
            Pedido pedido = _mapper.Map<Pedido>(transaction).Map(produtos);
            pedido.Situacao = PedidoSituacaoConstant.AGUARDANDO_PAGAMENTO;

            _pedidoRepository.Cadastrar(pedido);

            TransactionProduto tp = new TransactionProduto { Transaction = transaction, Produtos = produtos };
            PedidoSituacao pedidoSituacao = _mapper.Map<PedidoSituacao>(pedido).Map(tp);
